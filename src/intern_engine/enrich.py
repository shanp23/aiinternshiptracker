"""Enrichment: attach posting text to matched roles and classify coding needs.
...
async def enrich_jobs(jobs: list[Job], existing: dict, net: Net) -> tuple[set[str], int]:
    ...
    async def _resolve(job: Job) -> str | None:
        ...
        if prior.get("enriched_at") or prior.get("coding", "unknown") != "unknown":
            job.coding = prior.get("coding", "unknown")
        ...
        job.coding = coding.classify(job.description)
        return job.id
