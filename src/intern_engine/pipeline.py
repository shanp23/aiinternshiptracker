...
def _keep_matching(results, cfg, blocklist) -> tuple[list, set[str], int, Counter]:
    ...
    scope = cfg.get("role_scope", "ai-business")
    ...
    for company, jobs, error in results:
        ...
        for job in jobs:
            if not filters.is_internship(job.title):
                continue
            if scope == "ai-business" and not filters.is_ai_business(job.title):
                continue
            if scope == "tech" and not filters.is_tech(job.title):
                continue
            season = filters.detect_season(job.title, cycles)
            if season is None:
                continue
            tier = filters.loc_tier(job.location)
            if scope == "ai-business":
                if not tier:
                    continue  # not Triangle and not remote -> drop
                # remote roles that are explicitly foreign-scoped are dropped
                if tier == "remote":
                    loc = job.location or ""
                    if not filters.is_united_states(loc) and filters.is_canada(loc):
                        continue
                job.loc_tier = tier
            else:
            ...
            if cutoff and posted_day and posted_day < cutoff:
                continue
            job.season = season
            job.category = (filters.categorize_ai_business(job.title)
                            if scope == "ai-business" else filters.categorize(job.title))
            kept.append(job)
...
def run_update() -> tuple[dict, dict, list[str]]:
    ...
    async def _enrich_stage(results, net, workday_net):
        ...
        # STRICT no-coding mode: a posting whose body demands prior programming
        # is dropped entirely — it never reaches the store or the README.
        kept = [j for j in kept if j.coding != "coding-required"]
