...
def _row(record: dict) -> str:
    ...
    badges = " ".join(
        b for b in (
            "🏠" if record.get("loc_tier") == "triangle" else "",
            coding.flag(record.get("coding")),
...
def _header(cfg: dict, total_open: int, companies: int, new_week: int) -> list[str]:
    ...
    return [
        ...
        "- **No-code intel, computed** - 📚 will-train / 🧪 tools-preferred badges detected automatically from every "
...
def _select(rows: list[dict], limit, per_company) -> list[dict]:
    ...
    # Triangle roles pin to the top of each section, newest-first within tier.
    return sorted(
        rows,
        key=lambda r: (r.get("loc_tier") == "triangle", _date_str(r)[:10]),
        reverse=True,
    )
...
def _write_csv(open_jobs: list[dict]) -> None:
    fields = [
        ...
        "coding", "loc_tier", "salary", "posted_at", "first_seen_at", "url",
    ]
